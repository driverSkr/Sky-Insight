package com.ethan.base.widget.switchWidget
...
class SwitchButton @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0, defStyleRes: Int = 0) : View(context, attrs, defStyleAttr, defStyleRes) {
    ...
    private var aspectRatio: Float = 0.68f
    ...
    /** 开启状态背景色 */
    private var accentColor: Int = Color.parseColor("#6B57FF")

    /** 开启状态按钮描边色 */
    private var primaryDarkColor: Int = Color.TRANSPARENT

    /** 关闭状态描边色 */
    private var offColor: Int = Color.TRANSPARENT

    /** 关闭状态按钮描边色 */
    private var offDarkColor: Int = Color.parseColor("#9F9FA1")
    ...
    init {
        ...
        aspectRatio = array.getFloat(R.styleable.SwitchButton_aspectRatio, 0.68f)
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        when (MeasureSpec.getMode(finalWidthMeasureSpec)) {
            MeasureSpec.AT_MOST, MeasureSpec.UNSPECIFIED -> {
                finalWidthMeasureSpec = MeasureSpec.makeMeasureSpec((resources.getDimension(R.dimen.dp_56) + paddingLeft + paddingRight).toInt(), MeasureSpec.EXACTLY)
            }
        ...
        when (MeasureSpec.getMode(finalHeightMeasureSpec)) {
            MeasureSpec.AT_MOST, MeasureSpec.UNSPECIFIED -> {
                finalHeightMeasureSpec = MeasureSpec.makeMeasureSpec(((MeasureSpec.getSize(finalWidthMeasureSpec) * aspectRatio).toInt() + paddingTop + paddingBottom), MeasureSpec.EXACTLY)
            }
    ...
    override fun onSizeChanged(width: Int, height: Int, oldWidth: Int, oldHeight: Int) {
        ...
        if (canVisibleDrawing) {
            ...
            shadowGradient = RadialGradient(bCenterX, bCenterY, radius, Color.argb(200, red, green, blue), Color.argb(25, red, green, blue), Shader.TileMode.CLAMP)
        }
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (isEnabled) {
            paint.color = Color.parseColor("#9F9FA1")
        } else {
